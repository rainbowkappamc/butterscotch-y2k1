...
static void parseCommandLineArgs(CommandLineArgs* args, int argc, char* argv[]) {
    ...
    if (optind >= argc) {
        args->dataWinPath = "assets/data.win";
    } else {
...
// ===[ ENGINE FSM ]===
typedef enum {
    ENGINE_STATE_INIT,  /* Asset setup / compilation — game must not step */
    ENGINE_STATE_IDLE,  /* Main game loop — normal play */
    ENGINE_STATE_DEBUG  /* Debug state */
} EngineState;

static const char* EngineState_name(EngineState s) {
    switch (s) {
        case ENGINE_STATE_INIT:  return "INIT";
        case ENGINE_STATE_IDLE:  return "IDLE";
        case ENGINE_STATE_DEBUG: return "DEBUG";
        default:                 return "UNKNOWN";
    }
}

#define ENGINE_SET_STATE(var, newState) \
    do { \
        fprintf(stderr, "[ENGINE FSM] %s -> %s\n", EngineState_name(var), EngineState_name(newState)); \
        (var) = (newState); \
    } while (0)
...
int main(int argc, char* argv[]) {
    ...
    FILE* f = fopen(args.dataWinPath, "rb");
    if (f) {
        fclose(f);
    ...
    // Build window title
    char windowTitle[256];
    if (gen8) snprintf(windowTitle, sizeof(windowTitle), "Butterscotch - %s", gen8->displayName);

    // Initialize VM
    VMContext* vm = dataWin ? VM_create(dataWin) : nullptr;

    if (vm) Profiler_setEnabled(&vm->profiler, args.profilerFramesBetween > 0);
#ifdef ENABLE_VM_OPCODE_PROFILER
    if (vm) {
    ...
    if (args.hasSeed) {
        ...
        if (vm) vm->hasFixedSeed = true;
        printf("Using fixed RNG seed: %d\n", args.seed);
    }

    if (!dataWin) goto SKIP_DATAWIN_INIT;
...
SKIP_DATAWIN_INIT:;
    OverlayFileSystem* overlayFs = nullptr;
    if (dataWin) {
    const char* savePath2 = args.saveFolder != nullptr ? args.saveFolder : dataWinDir;
    overlayFs = OverlayFileSystem_create(dataWinDir, savePath2);
    free(dataWinDir);
    }

    // Init GLFW — already done above, skip re-init
#ifndef USE_GLFW2
...
#ifdef USE_GLFW2
    window = glfwOpenWindow((int) gen8->defaultWindowWidth, (int) gen8->defaultWindowHeight, 8, 8, 8, 8, 24, 8, GLFW_WINDOW);
#endif
    if (!window) {
    ...
    Runner* runner = dataWin ? Runner_create(dataWin, vm, renderer, (FileSystem*) overlayFs, audioSystem) : nullptr;
    if (runner) {
    ...
    EngineState engineState = ENGINE_STATE_INIT;
    fprintf(stderr, "[ENGINE FSM] Initial state: %s\n", EngineState_name(engineState));
    if (gen8) DummyGraphics_init((int) gen8->defaultWindowWidth, (int) gen8->defaultWindowHeight);
    else DummyGraphics_init(640, 480);
    int InitOffset = 3;
    double initStart = glfwGetTime();
    while (!dataWin || glfwGetTime() - initStart < (double)InitOffset) {
        glfwPollEvents();

        /* F1 pressed in INIT -> go to DEBUG */
        if (glfwGetKey(window, GLFW_KEY_F1) == GLFW_PRESS) {
            ENGINE_SET_STATE(engineState, ENGINE_STATE_DEBUG);
            break;
        }

        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glClear(GL_COLOR_BUFFER_BIT);
        loadimage("dummy", 0, 0, 0);
#ifdef USE_GLFW2
        glfwSwapBuffers();
#else
    ...
    if (engineState == ENGINE_STATE_DEBUG) {
        /* Unload dummy — clear to black */
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glfwSwapBuffers(window);
#endif
        DummyGraphics_shutdown();

#ifndef USE_GLFW2
        DebugResult dbResult = DebugScreen_run(runner, window, audioSystem, argc, argv);
        if (runner && dbResult.action == DEBUG_ACTION_ENTER_ROOM && dbResult.roomIndex >= 0) {
            Runner_initFirstRoom(runner);
            runner->pendingRoom = dbResult.roomIndex;
            ENGINE_SET_STATE(engineState, ENGINE_STATE_IDLE);
        } else {
            goto CLEANUP;
        }
#else
    ...
    while (true) {
        ...
        if (engineState == ENGINE_STATE_IDLE && shouldStep) {
            if (args.traceFrames) {
    ...
    // Cleanup
CLEANUP:
    if (runner) { runner->audioSystem->vtable->destroy(runner->audioSystem); runner->audioSystem = nullptr; }
    else if (audioSystem) { audioSystem->vtable->destroy(audioSystem); }
    if (renderer) renderer->vtable->destroy(renderer);
    ...
    if (overlayFs) OverlayFileSystem_destroy(overlayFs);
#ifdef ENABLE_VM_OPCODE_PROFILER
    if (vm) VM_printOpcodeProfilerReport(vm);
#endif
    if (vm) VM_free(vm);
